...

	{
...

		{
...

			{
				...
				$template = $this->base_model->fetch_records_from('templates', array('template_key' => 'Forgot Password', 'template_status' => 'Active'));
				if(!empty($template))
				{
					$data['site_theme'] = 'seat';
					//$message = $this->load->view($this->config->item('email_templates', 'ion_auth').$this->config->item('email_forgot_password', 'ion_auth'), $data, true);
					$message = $template[0]->template_content;
					$link = base_url().'auth/reset_password/'.$data['forgotten_password_code'];
					...
					{
					$sub = $this->config->item('site_settings')->site_title . ' - ' . $this->lang->line('email_forgotten_password_subject');
					}
...

	{
...

		{
...

			{
				...
				{
					$message = $template[0]->template_content;
					$message = str_replace('__USER_NAME__', $data['identity'], $message);
					$message = str_replace('__PASSWORD__', $data['new_password'], $message);
					//$message = str_replace('__LINK_TITLE__', $link, $message);
		...
		else
		{
			...
			else
			{
				if(in_array(2, $group_ids)) {
					...
					$template = $this->base_model->fetch_records_from('templates', array('template_key' => 'Registration Activation', 'template_status' => 'Active'));
					...
					$message = str_replace('__PASSWORD__', $data['password'], $message);
					else
					$message = str_replace('__PASSWORD__', '*********', $message);	
					$link = '<a href="'.base_url().'auth/activate/'.$id.'/'.$data['activation'].'">'.lang('email_activate_link').'</a>';
					$message = str_replace('__LINK_TITLE__', $link, $message);
					...
					{
					$sub = $this->config->item('site_settings')->site_title . ' - ' . $this->lang->line('email_activation_subject');
					}
...

{
...

	{
		...
		$executive_group = $this->config->item('executive_group', 'ion_auth');



		return $this->in_group($executive_group, $id);
	...
	public function is_b2buser($id=false)

	{

		$this->ion_auth_model->trigger_events('is_b2buser');



		$b2buser_group = $this->config->item('b2buser_group', 'ion_auth');



		return $this->in_group($b2buser_group, $id);
	...
	public function is_supplier($id=false)

	{

		$this->ion_auth_model->trigger_events('is_supplier');



		$supplier_group = $this->config->item('supplier_group', 'ion_auth');



		return $this->in_group($supplier_group, $id);
...

	{
		...
		$this->ion_auth_model->trigger_events('is_driver');

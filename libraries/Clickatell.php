...
class Clickatell
{
    ...
    public function __construct()
    {
		...
		$query = 'SELECT * FROM '.$this->ci->db->dbprefix('gateways').' g INNER JOIN '.$this->ci->db->dbprefix('gateways_fields').' gf ON g.`gateway_id`=gf.`gateway_id` LEFT JOIN '.$this->ci->db->dbprefix('gateways_fields_values').' gfv ON gf.`field_id` = gfv.`gateway_field_id` WHERE g.`gateway_title`="Cliakatell" ORDER BY gf.field_order ASC';		
		...
		if(count($gateway_details) > 0) {
			foreach($gateway_details as $selectedgateway) {
				switch($selectedgateway->field_key)
				{
					...
					break;
					case 'api_id':
						$this->api_id = $selectedgateway->gateway_field_value;
					break;
					case 'from_no':
						$this->from_no = $selectedgateway->gateway_field_value;
					break;
				}						
			}
		}

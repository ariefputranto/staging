...
class Plivo
{
    ...
    public function __construct()
    {
		...
		*/
		...
		$query = 'SELECT * FROM '.$this->_ci->db->dbprefix('gateways').' g INNER JOIN '.$this->_ci->db->dbprefix('gateways_fields').' gf ON g.`gateway_id`=gf.`gateway_id` LEFT JOIN '.$this->_ci->db->dbprefix('gateways_fields_values').' gfv ON gf.`field_id` = gfv.`gateway_field_id` WHERE g.`gateway_title`="Plivo" ORDER BY gf.field_order ASC';		
		...
		if(count($gateway_details) > 0) {
			foreach($gateway_details as $selectedgateway) {
				...
				{
					case 'auth_id':
						$this->auth_id = $selectedgateway->gateway_field_value;
					break;
					case 'auth_token':
						$this->auth_token = $selectedgateway->gateway_field_value;
					break;
					case 'api_version':
						$this->api_version = $selectedgateway->gateway_field_value;
					break;
					case 'end_point':
						$this->end_point = $selectedgateway->gateway_field_value;
					break;
				}						
			}
		}

...
class Managedriver extends MY_Controller
				...
				if ($this->form_validation->run() == true) {
					...
					if(!empty($shuttles))
					{
						...
						foreach($shuttles as $shuttle)
						{
							...
							{
								$this->base_model->update_operation(array('driver_id' => $data['driver_id'], 'updated' => date('Y-m-d H:i:s')), 'travel_location_costs_drivers', array('id' => $check[0]->id));
							}
						}
						if(!empty($newdata))
						{

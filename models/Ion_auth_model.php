...
class Ion_auth_model extends CI_Model
{
	...
	public function __construct()
	{
		...
		$this->customer_column = $this->config->item('customer_id', 'ion_auth');
	...
	/**
	* 
	* @Activate Customer or Employee
	* 
	* @return
	* @author John Peter
	*/
	public function cactivate($customer_id,$code=false)
	{
		$this->trigger_events('pre_activate');

		if ($code !== FALSE)
		{
			$query = $this->db->select($this->customer_column)
			                  ->where('activation_code', $code)
			                  ->limit(1)
			                  ->get($this->tables['buser_customers']);
			...
			$identity = $result->{$this->customer_column};
			...
			$this->trigger_events('extra_where');
			$this->db->update($this->tables['buser_customers'], $data, array($this->customer_column => $customer_id));
		}
		else
		{
			$data = array(
			...
			$this->trigger_events('extra_where');
			$this->db->update($this->tables['buser_customers'], $data, array('customer_id' => $customer_id));
		}
	...
	*/
	...
	{
		$this->trigger_events('cdeactivate');
		...
		$this->trigger_events('extra_where');
		$this->db->update($this->tables['buser_customers'], $data, array('customer_id' => $id));
...
//print_r($data);die();
		...
		if ($return)
		{
			$template = $this->base_model->fetch_records_from('templates', array('template_key' => 'New Password', 'template_status' => 'Active'));
			//print_r($template);die();
			if(!empty($template))
			{
				$message = $template[0]->template_content;
				$message = str_replace('__USER_NAME__', $identity, $message);
				$message = str_replace('__PASSWORD__', $old_password, $message);
				...
				{
				$sub = $this->config->item('site_settings')->site_title . ' - ' . $this->lang->line('email_new_password_subject');
				}
	...
	public function forgotten_password($identity)
	{
		...
		$key = str_replace('/','.', $key); //If the above function generates any '/', then it will be replace with '.' to avoid URL issues
	...
	public function login($identity, $password, $remember=FALSE)
	{
		...
		$query = $this->db->select($this->identity_column . ', username, first_name, email, phone, phone_code, id, password, active, last_login, photo')
	 ...
	 * @author Mathew
	 **/
	 public function customer_login($customer_id, $password, $remember=FALSE)
	 {
	 	$this->trigger_events('pre_login');
		...
		$query = $this->db->select($this->customer_column . ', name, email_id, buser_id, password,customer_type ,active, last_login')
		                  ->where($this->customer_column, $this->db->escape_str($customer_id))
		                  ->limit(1)
		                  ->get($this->tables['buser_customers']);

		if($this->is_time_locked_out($customer_id))
		{
			//Hash something anyway, just to take up time
			//$this->hash_password($password);
		...
		if ($query->num_rows() === 1)
		{
			...
			if ($password === $user->password)
			{
				...
				$this->set_customer_session($user);

				$this->update_customer_last_login($user->customer_id);

				$this->clear_login_attempts($customer_id);

				/*if ($remember && $this->config->item('remember_users', 'ion_auth'))
				{
					$this->remember_user($user->customer_id);
				}*/
		...
		$this->increase_login_attempts($customer_id);
	...
	*/
	...
	{
		$this->trigger_events('customer');

		//if no id was passed use the current users id
		$customer_id || $customer_id = $this->session->userdata('customer_id');

	$this->limit(1);
		$this->where($this->tables['buser_customers'].'.customer_id', $customer_id)->get();
	...
	**/
	...
	public function update_customer_last_login($customer_id)
	{
		$this->trigger_events('update_customer_last_login');

		$this->load->helper('date');

		$this->trigger_events('extra_where');

		$this->db->update($this->tables['buser_customers'], array('last_login' => time()), array('customer_id' => $customer_id));
	...
	public function set_customer_session($user)
	{
		$this->trigger_events('pre_set_session');

		$session_data = array(
		    'identity'        	   => $user->{$this->customer_column},
		    'customer_id'          => $user->{$this->customer_column},
		    'username'             => $user->name,
		    'email'                => $user->email_id,
		    'buser_id'             => $user->buser_id, //everyone likes to overwrite id so we'll use user_id
		    'old_last_login'       => $user->last_login,
...
* @author John Peter
*/
public function remember_customer($customer_id)
{
	$this->trigger_events('pre_remember_user');
		...
		$customer = $this->customer($customer_id)->row();

		$salt = $this->salt();

		$this->db->update($this->tables['buser_customers'], array('remember_code' => $salt), array('customer_id' => $customer_id));
		...
		if ($this->db->affected_rows() > -1)
		{
			...
			set_cookie(array(
			    'name'   => 'customer_id',
			    'value'  => $customer->{$this->customer_column},
			    'expire' => $expire
			));
	...
	public function login_remembered_user()
	{
		...
		//get the user
		$this->trigger_events('extra_where');
		$query = $this->db->select($this->identity_column.', id, username, first_name, email, active, last_login, photo')

...
class sendsms
{
	...
	function __construct($method = 'sms')
	{
		...
		$query = 'SELECT * FROM '.$this->_ci->db->dbprefix('gateways').' g INNER JOIN '.$this->_ci->db->dbprefix('gateways_fields').' gf ON g.`gateway_id`=gf.`gateway_id` LEFT JOIN '.$this->_ci->db->dbprefix('gateways_fields_values').' gfv ON gf.`field_id` = gfv.`gateway_field_id` WHERE g.`gateway_title`="Solutionsinfini" ORDER BY gf.field_order ASC';		
		...
		if(count($gateway_details) > 0) {
			foreach($gateway_details as $selectedgateway) {
				...
				{
					case 'working_key':
						$wk = $selectedgateway->gateway_field_value;
					break;
					case 'sender_id':
						$sd = $selectedgateway->gateway_field_value;
					break;
					case 'api':
						$apiurl = $selectedgateway->gateway_field_value;
					break;
				}						
			}
		}

      ...
      <div class="admin-body">
		  ...
		  <div class="col-md-6">
			...
			<div class="form-group">
			  <?php 
					$sel = '';
					if(isset($currentGroups[0]->id))
						$sel = $currentGroups[0]->id;
					echo form_dropdown('group', $group_opts, $sel, 'id="group" onchange="toggleAmtDiv(this.value);"').form_error('group'); 
				?>
			</div>

			<div class="form-group" id="amt_field_div" style="display: none;">
			  <?php echo form_input($deposited_amount); ?>
			...
			<input type="hidden" name="id" value="<?php echo $user->id;?>">
			 ...
			 <div class="form-group">
			<button class="btn btn-success" type="submit" ><?php if(isset($this->phrases["update"])) echo $this->phrases["update"]; else echo "Update";?></button>
			</div>
			 </form>
...
<script>
          ...
          setupFormValidation: function()
          {
			...
			/* Create Account form validation rules */
              $("#register_form").validate({
				...
				messages: {
					first_name: {
							  required: "<?php if(isset($this->phrases["please enter first name"])) echo $this->phrases["please enter first name"]; else echo "Please enter First Name";?>."
						  },
					email: {
							  required: "<?php if(isset($this->phrases["please enter email id"])) echo $this->phrases["please enter email id"]; else echo "Please enter Email-id";?>."
						  },
					phone_code: {
   ...
   $(document).ready(function() {
   		toggleAmtDiv($('#group option:selected').val());
   });

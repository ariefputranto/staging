      ...
      <div class="admin-body">
		  <div class="inner-elements">
			...
			<?php
				...
				echo form_open('settings/change_vehicle', $attributes);
				?>
		  <div class="col-md-6">
			<div class="form-group">
			   ...
			   if(isset($_POST['show']) || isset($_POST['change']))
				  $val = $_POST['present_vehicle'];
			  if(isset($_POST['change']))
			   ?>
			   <?php echo form_dropdown('present_vehicle', $vehicles, $val, 'class="chzn-select"');?> Changes with 
               <?php echo form_error('present_vehicle'); ?>
			</div>
		 ...
		 <div class="col-md-6">
			<div class="form-group">
			   ...
			   $val = '';
			   if(isset($_POST['show']) || isset($_POST['change']))
				  $val = $_POST['new_vehicle'];
			   ?>
			   <?php echo form_dropdown('new_vehicle', $vehicles, $val, 'class="chzn-select"');?>
               <?php echo form_error('new_vehicle'); ?>
			</div>
		...
		?>
		 <div class="col-md-12">
			<div class="form-group">
			   ...
			   {
			   ?>
			   <tr><td colspan="5">Shuttles of <b><?php echo $details[0]->vehicle_name.' '.$details[0]->vehicle_model . ' ('.$details[0]->category.')';?></b></td></tr>
			   ...
			   <?php
				...
				foreach($details as $d)
				{
					...
					if(in_array($d->id, $assigned_shuttles))
						$checked = ' checked';
					echo '<tr><td>'.++$i.'</td><td>'.$d->shuttle_no.'</td><td>'.$d->vehicle_name.'</td><td>'.$d->start_time.' to '.$d->destination_time.'</td><td><input type="checkbox" name="shuttles[]" value="'.$d->id.'_'.$d->driver_id.'"'.$checked.'>'.form_error('shuttles[]').'</td></tr>';
				}				
			   }
			   else
			   {
			...
			if(isset($_POST['change']))
				  $val = $_POST['present_vehicle'];
			?>
			<input type="hidden" name="present_vehicle" value="<?php echo $val;?>">
			...
			if(isset($_POST['change']))
				  $val = $_POST['new_vehicle'];
			?>
			<input type="hidden" name="new_vehicle" value="<?php echo $val;?>">
          ...
          setupFormValidation: function()
          {
			/* Create Account form validation rules */
              $("#register_form").validate({
				...
				messages: {
					new_vehicle: {
							  required: "<?php if(!empty($this->phrases["please select vehicle"])) echo $this->phrases["please select vehicle"]; else echo "Please select vehicle";?>."
						  },
					special_start: {

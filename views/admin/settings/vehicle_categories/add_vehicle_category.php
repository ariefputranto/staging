		  ...
		  <div class="inner-elements">
			...
			<?php echo $this->session->flashdata('message');?>
				<?php 
				  ...
				  echo form_open('settings/vehicleCategories/'.$param,$attributes);?> 

				<?php if(count($records) > 0) $record = $records[0]; 
					?>
				<div class="form-group">
					<label><?php if(isset($this->phrases["category"])) echo $this->phrases["category"]; else echo "Category";?>&nbsp;<font color="red">*</font></label>
					 <input type="text" name="category" id="category" value="<?php echo set_value('category', (isset($record->category)) ? $record->category : '');?>"/>
				...
				<div class="form-group">	  
					...
					<a onclick="window.location.href = '<?php echo base_url().'settings/vehicleCategories/list';?>';" class="btn btn-info" title="<?php if(isset($this->phrases["cancel"])) echo $this->phrases["cancel"]; else echo "Cancel";?>"> <?php if(isset($this->phrases["cancel"])) echo $this->phrases["cancel"]; else echo "Cancel";?></a>
				</div>
				</form>
			</div>
          ...
          setupFormValidation: function()
          {
			...
			/* Form validation rules */
              $("#formm").validate({
				...
				messages: {
					category: {
							  required: "<?php if(isset($this->phrases["please enter category name"])) echo $this->phrases["please enter category name"]; else echo "Please enter category name";?>."
						  }
				},

      ...
      <div class="admin-body">
		  ...
		  <?php echo $this->session->flashdata('message');?>
				<?php 
				  ...
				  echo form_open('admin/templates/'.$param,$attributes);?> 
			...
			<div class="col-md-8">
				...
				<div class="form-group">
					<label><?php if(!empty($this->phrases["subject"])) echo $this->phrases["subject"]; else echo "Subject";?></label>
					 <input type="text" name="template_subject" id="template_subject" value="<?php echo set_value('template_subject', (isset($record->template_subject)) ? $record->template_subject : '');?>"/>
					<?php echo form_error('template_subject'); ?>
				</div>
				
				<div class="form-group">
					<label><?php if(!empty($this->phrases["header"])) echo $this->phrases["header"]; else echo "Header";?></label>
					 <textarea class="editor" name="template_header" id="template_header"><?php echo set_value('template_header', (isset($record->template_header)) ? $record->template_header : '');?></textarea>
				...
				<?php if(isset($record->template_variables)) { ?>
				<div class="form-group">
					<label><?php if(!empty($this->phrases["allowed variables"])) echo $this->phrases["allowed variables"]; else echo "Allowed Variables";?></label>
					 <?php echo $record->template_variables;?>
				</div>
				<?php } ?>
				
				<div class="form-group">
					<label><?php if(!empty($this->phrases["footer"])) echo $this->phrases["footer"]; else echo "Footer";?></label>
					 <textarea class="editor" name="template_footer" id="template_footer"><?php echo set_value('template_footer', (isset($record->template_footer)) ? $record->template_footer : '');?></textarea>
					<?php echo form_error('template_footer'); ?>
				</div>
			   ...
			   <input type="hidden" name="update_rec_id" value="<?php if(isset($record->template_id)) echo $record->template_id;?>" />
				...
				<div class="form-group">	  
					...
					<a onclick="window.location.href = '<?php echo base_url().'admin/templates/list';?>';" class="btn btn-info" title="<?php if(!empty($this->phrases["cancel"])) echo $this->phrases["cancel"]; else echo "Cancel";?>"> <?php if(!empty($this->phrases["cancel"])) echo $this->phrases["cancel"]; else echo "Cancel";?></a>
				</div>
				</form>

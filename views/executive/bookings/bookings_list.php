                    ...
                    <tr>
                      ...
                      <tbody>
						...
						<tr <?php echo $class;?>>
                          ...
                          <td>
							<a <?php if($row->read_status == '0') { ?>onclick="updateReadStatus(<?php echo $row->id;?>); window.location.href = '<?php echo base_url().'executive/viewBookings/details/'.$row->id;?>';" <?php } else { ?> onclick="window.location.href = '<?php echo base_url().'executive/viewBookings/details/'.$row->id;?>';" <?php }?> class="btn btn-info act-btn" title="<?php if(isset($this->phrases["view details"])) echo $this->phrases["view details"]; else echo "View Details";?>"><i class="fa fa-eye"></i> </a>
...
/****** Cancel Message ******/
  ...
  function updateReadStatus(id)
  {
	if(id > 0){
		...
		$.ajax({
			  type: "post",
			  url: "<?php echo base_url();?>executive/updateReadStatus",
			  async: false,

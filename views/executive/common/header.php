...
<link href="<?php echo base_url().'vehicle/';?>assets/system_design/css/jquery.dataTables.css" rel="stylesheet">
<?php } ?>
...
<section class="top_wrapper">
  ...
  <div class="header admin-header">
    <div class="navbar-header">
      ...
      <div class="navbar-brand lg">
        <div class="logo"> <a href="<?php echo base_url();?>executive"><img height="50" src="<?php echo base_url().$site_theme.'/';?>assets/system_design/images/<?php if($site_settings->site_logo != "" && file_exists($site_theme.'/'.'assets/system_design/images/'.$site_settings->site_logo)) echo $site_settings->site_logo; else echo "logo.png";?>"></a> </div>
        <!--./logo--></div>
    </div>
  ...
  <div class="dropdown ad">
        <ul class="message_ul">
          <li><a title="<?php if(isset($this->phrases["today's bookings"])) echo $this->phrases["today's bookings"]; else echo "Today's Bookings";?>" href="<?php echo base_url();?>executive/viewBookings/todayz" class="round_div" ><i class="fa fa-envelope"></i><span class="badge bg-success"><?php echo $this->config->item('count_of_todayz_bookings');?></span></a>
          </li>
  		...
  		<?php
				...
				$languages = $this->db->get_where('languages', array('status' => 'Active'))->result();

				$first_opt = (isset($this->phrases["change site language"])) ? $this->phrases["change site language"] : "Change Site Language";

				$langOptions['']	=	$first_opt; 

				foreach($languages as $lang) {

					$langOptions[$lang->id]	=	ucwords($lang->lang_name); 

				}

				echo form_dropdown('language', $langOptions, $site_settings->language_id, 'class="chzn-select cs" id="language" onchange="changeLanguage(this.value)"');

				?> 
   <ul class="navbar-right login user_proile">
        <li><a href="#"><img height="38" src="<?php echo base_url();?>uploads/admin_profile_pic/<?php if($this->session->userdata('photo') != "" && file_exists('uploads/admin_profile_pic/'.$this->session->userdata('photo'))) echo $this->session->userdata('photo'); else echo "noimage.jpg";?>"> <?php echo $this->session->userdata('username')?></a>
          <ul class="dropdown-menu" role="menu" aria-labelledby="dLabel">
            <li><a tabindex="-1" href="<?php echo base_url();?>executive/profile"> <i class="fa fa-user"></i> <?php if(isset($this->phrases["my profile"])) echo $this->phrases["my profile"]; else echo "My Profile";?></a></li>
            <li><a tabindex="-1" href="<?php echo base_url();?>executive/change_password"> <i class="fa fa-newspaper-o"></i> <?php if(isset($this->phrases["change password"])) echo $this->phrases["change password"]; else echo "Change Password";?></a></li>
